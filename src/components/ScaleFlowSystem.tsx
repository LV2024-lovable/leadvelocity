        ...
        <div className="relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 relative z-10">
            {steps.map((step, index) => (
              <div key={step.id} className={`flex flex-col ${index % 2 === 0 ? 'animate-fade-in' : 'animate-fade-in'}`} style={{animationDelay: `${index * 0.2}s`}}>
                ...
                {index < steps.length - 1 && (
                  <div className="hidden lg:block absolute top-1/2 right-0 transform -translate-y-1/2 translate-x-1/2 z-20 w-4 h-4 bg-velocity-blue rounded-full border-2 border-white shadow-md" style={{ left: `${(100 / (steps.length - 1)) * (index + 0.5)}%` }} />
                )}
              </div>
            ))}
          </div>
          ...
          <div className="hidden lg:flex justify-between absolute top-1/2 left-[18%] right-[18%] transform -translate-y-1/2 z-0 px-6">
            {[...Array(steps.length - 1)].map((_, i) => (
              <svg key={i} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-velocity-blue">
